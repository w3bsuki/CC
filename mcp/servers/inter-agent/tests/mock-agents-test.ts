import { InterAgentServer } from '../src/inter-agent-server';

// Mock agent implementation
class MockAgent {
  constructor(
    private server: InterAgentServer,
    private id: string,
    private role: any
  ) {}
...
async function testInterAgentCommunication() {
  ...
  console.log('\n🔍 Test 5: Agent Discovery');
  const availableAgents = server.getAvailableAgents();
  console.log('Available agents:', availableAgents.map(a => `${a.id} (${a.role})`));
  
  const taskAgents = server.getAgentsByRole('task');
  console.log('Task agents:', taskAgents.map(a => a.id));
